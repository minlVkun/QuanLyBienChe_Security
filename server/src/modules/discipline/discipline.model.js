...
class DisciplineModel {
    ...
    static async getByMaNV(reqUser, maNV) {
        const query = `
            SELECT 
                ID_KTKL AS RecordID, MaNV, Loai, HinhThuc, NgayQuyetDinh, SoQuyetDinh, NoiDung
    ...
    static async delete(reqUser, id) {
        const query = `
            DELETE FROM [HR].[KhenThuongKyLuat] 
            WHERE ID_KTKL = @ID_KTKL;
            SELECT @@ROWCOUNT AS AffectedRows;
        `;
        const inputs = [
            { name: 'ID_KTKL', type: sql.Int, value: id }
        ];
